...
func (c *Cache) notifyBlockingQuery(ctx context.Context, r getOptions, correlationID string, cb Callback) {
	...
	for {
		...
		index, failures = c.notifyBlockingQueryOnce(ctx, r, correlationID, cb, index, failures)
	}
}
func (c *Cache) notifyBlockingQueryOnce(
	ctx context.Context,
	r getOptions,
	correlationID string,
	cb Callback,
	currIndex uint64,
	currFailures uint,
) (index uint64, failures uint) {
	// Seed response data from last execution.
	index = currIndex
	failures = currFailures

	// Ensure that each invocation terminates instead of stacking.
...
func (c *Cache) notifyPollingQuery(ctx context.Context, r getOptions, correlationID string, cb Callback) {
	...
	for {
		...
		index, failures, lastValue = c.notifyPollingQueryOnce(ctx, r, correlationID, cb, index, failures, lastValue)
	}
}
func (c *Cache) notifyPollingQueryOnce(
	ctx context.Context,
	r getOptions,
	correlationID string,
	cb Callback,
	...
	currFailures uint,
	currValue any,
) (index uint64, failures uint, lastValue any) {
	// Seed response data from last execution.
	index = currIndex
	failures = currFailures
	lastValue = currValue
