...
func TestGenerateConfig(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			if tc.TLSServer {
				srv = httptest.NewTLSServer(testMockAgent(t, tc))
			} else {
				srv = httptest.NewServer(testMockAgent(t, tc))
			}
			defer srv.Close()
...
func testMockAgent(t *testing.T, tc generateConfigTestCase) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.Contains(r.URL.Path, "/agent/self"):
			testMockAgentSelf(t, tc.XDSPorts, tc.AgentSelf110)(w, r)
...
func testMockAgentSelf(t *testing.T, wantXDSPorts agent.GRPCPorts, agentSelf110 bool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		resp := agent.Self{
